...
export const createimpressaPDF = ({ title, contentBuilder, signatory, logoPath }) => {
  ...
  const drawHeader = () => {
    ...
    doc.text("impressa Custom Solutions", left, headerStartY, {
      width: innerWidth,
      align: "center",
      lineBreak: false
    });
  ...
  const drawFooter = () => {
    ...
    doc.text("impressa Custom Solutions | Kigali, Rwanda", left, footerY + 5, {
      width: innerWidth / 3,
      align: "left",
      lineBreak: false
    });
    doc.text("info@impressa.rw | +250 788 000 000", left + innerWidth / 3, footerY + 5, {
      width: innerWidth / 3,
      align: "center",
      lineBreak: false
    });
  ...
  doc.text(`Title: ${signatory.title || "impressa Administrator"}`);
  doc.moveDown(1);
